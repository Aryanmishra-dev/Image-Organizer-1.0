...
def compute_xxhash(path: Path, chunk_size: int = CHUNK_SIZE) -> Optional[str]:
    """
...
def compute_dhash(path: Path, hash_size: int = 16) -> Optional[str]:
    """
...
def compute_average_hash(path: Path, hash_size: int = 16) -> Optional[str]:
...
class ParallelHasher:
    ...
    def hash_files(
        ...
        args_list = [
            (meta.path, self.compute_perceptual, self.use_xxhash)
            for meta in files
        ]
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(_hash_single_file, args): args[0]
