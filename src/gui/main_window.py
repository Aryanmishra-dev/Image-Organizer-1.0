...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.scan_result: ComparisonResult | None = None
    ...
    def _create_dashboard_view(self) -> QWidget:
        ...
        actions_label.setStyleSheet(
            f"color: {DARK_TEXT_PRIMARY}; margin-top: 20px; background: transparent;"
        )
    ...
    def _create_scan_view(self) -> QWidget:
        ...
        options_title.setStyleSheet(
            f"color: {DARK_TEXT_PRIMARY}; margin-bottom: 12px; background: transparent;"
        )
        ...
        self.keep_strategy.addItems(
            ["Newest File", "Oldest File", "Largest File", "Smallest File", "Best Quality"]
        )
        ...
        self.similarity_label.setStyleSheet(
            f"color: {DARK_TEXT_PRIMARY}; font-weight: bold; background: transparent;"
        )
        ...
        self.include_perceptual.setStyleSheet(
    ...
    def _create_settings_view(self) -> QWidget:
        ...
        # AI Settings
        ai_section = self._create_settings_section(
            "🤖 AI Analysis",
            [
    ...
    def _create_settings_section(self, title: str, settings: list) -> QFrame:
        ...
        for setting_name, default_value in settings:
            ...
            if isinstance(default_value, bool):
                ...
                input_field = (
                    QComboBox()
                    if default_value in ["medium", "high", "low"]
                    else QLabel(str(default_value))
                )
    ...
    def _quick_cleanup(self):
        ...
        if self.analysis_results:
            ...
            QMessageBox.information(
                self, "No Results", "Please run a scan and AI analysis first before cleanup."
            )
    ...
    def _start_scan(self):
        ...
        include_perceptual = self.include_perceptual.isChecked() and (
            self.detection_mode.currentIndex() >= 1
        )  # Similar or Both
    ...
    def _populate_scan_results(self, result: ComparisonResult):
        ...
        for idx, group in enumerate(all_groups):
            ...
            self.results_table.setItem(
                idx, 2, QTableWidgetItem(group.members[0].path.name if group.members else "")
            )
    ...
    def _start_analysis(self):
        ...
        groups_data = [{"files": [str(m.path) for m in g.members]} for g in all_groups]

        preferences = {
            "keep_strategy": self._get_keep_strategy_value(),
            "similarity_threshold": similarity_percent_to_hamming_threshold(
    ...
    def _on_analysis_complete(self, analysis: dict):
        ...
        for idx, rec in enumerate(recommendations):
            ...
            conf_item = QTableWidgetItem(
                f"{'🟢' if confidence == 'high' else '🟡' if confidence == 'medium' else '🔴'} {confidence}"
            )
            if confidence == "high":
    ...
    def _on_analysis_error(self, error: str):
        ...
        self.analyze_btn.setEnabled(True)
        QMessageBox.warning(
            self, "Analysis Error", f"AI analysis failed:\n{error}\n\nUsing rule-based fallback."
        )
    ...
    def _execute_plan(self):
        ...
        if not selected_recommendations:
            QMessageBox.warning(
                self, "No Selection", "Please select at least one group to clean up."
            )
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                current_saved = float(
                    self.space_saved_label.text().replace(" MB", "").replace(",", "")
                )
